...
tspan = (0.0, 20)
tspan_train=(0.0, 2.5)
u0 = Float32[-8.340883, -13.625923, 17.298455]


p_ = Float32[28.0, 2.666666666665, 10.0]
prob = ODEProblem(lorenz63!, u0, tspan, p_)

#generate training data, split into train/test
solution = solve(prob, Tsit5(), abstol = 1e-5, reltol = 1e-7, saveat = timestep)
...
function plot_save(l, l_test, p_list, epoch)
    ...
    pred_sol_true = solve(ODEProblem(lorenz63!, u0, tspan, p_), Tsit5(), abstol = 1e-5, reltol = 1e-7, saveat = timestep)
